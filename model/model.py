...
def build_model(cfg: dict = None,
    ...
    print("vocab_len="+str(len(src_vocab)))
    # Define source embedding
    model = FastText.load('FastText_large.model')
    weight_matrix=np.zeros((7547,256))

    for key in src_vocab.stoi.keys():
        embed=model.wv[key]
        weight_matrix[src_vocab.stoi[key]]=embed
    pretrained_embeddings = torch.tensor(weight_matrix, dtype=torch.float)
    src_embed = Embeddings(
        **cfg["encoder"]["embeddings"], vocab_size=len(src_vocab),
        padding_idx=src_padding_idx,weights=pretrained_embeddings)
